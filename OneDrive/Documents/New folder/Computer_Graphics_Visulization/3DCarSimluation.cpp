...
char name3[]="PROJECT:  3D CAR  SIMULATION";
...
void display1(void)
{
	...
	display_string(280,540,"RNS INSTITUTE OF TECHNOLOGY",1);
	display_string(290,500,name3,1);
	display_string(380,475,"-Divyansh Tiwari",2);
	display_string(420,440,"HELP",2);
	display_string(100,400,"MOUSE",2);
	display_string(100,360,"PRESS RIGHT BUTTON FOR MENU",3);
	display_string(100,320,"KEYBOARD",2);
	display_string(100,290,"X-Y-Z KEYS FOR CORRESPONDING ROTATION",3);
	display_string(100,260,"A-S-Q CAR CUSTOM SIZE SELECTION",3);
	display_string(100,230,"U-F FOR CAMERA VIEW SETTINGS",3);
	display_string(100,200,"USE LEFT ARROW(<-) AND RIGHT ARROW(->) TO MOVE CAR",3);
	display_string(100,170,"ESCAPE TO EXIT",3);
	display_string(320,100,"PRESS SPACE BAR TO ENTER",2);
	glutPostRedisplay();
	glutSwapBuffers();
...
GLvoid DrawGLScene()
{
  ...
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);	/* Cvlear The Screen And The Depth Buffer */
if(view==0)
{
init();
display1();
...
else
{
  ...
  if(flag2==1)
  {
  GLfloat fogcolour[4]={0.8,0.8,0.8,1.0};         
...
void myMenu(int id)
{
	...
	flag2=2;
	glClearColor(0.1,0.1,0.1,0);
	GLfloat fogcolour[4]={0.0,0.0,0.0,0.5};         
